...
Collaboration에 처음 액세스하면 계정을 설정하라는 메시지가 표시됩니다. 계정 세부 정보를 구성하고 키를 일치시킬 수 있는 일회성 프로세스입니다. 조직의 첫 번째 계정인 경우 [계정 세부 정보](#set-up-details) 설정을 시작으로 온보딩 프로세스를 즉시 안내합니다.

조직을 추가하려면 왼쪽 레일에서 **[!UICONTROL 설정]**(으)로 이동한 다음 추가 아이콘(![추가 아이콘](/help/assets/icons/plus.png))을 선택하십시오. 오른쪽 상단에 있습니다. 그런 다음 **[!UICONTROL 계정]**&#x200B;을 선택합니다.
...
* 브랜드를 명확하게 나타내는 **[!UICONTROL 계정 이름]**&#x200B;을 추가하십시오.
* 내 브랜드에 대한 **[!UICONTROL 설명]**&#x200B;을(를) 추가합니다. 이는 선택 사항이지만 다른 공동 작업자가 브랜드를 더 잘 이해하는 데 도움이 됩니다.
* **[!UICONTROL 역할]**&#x200B;을 선택하세요. **[!UICONTROL 광고주]**&#x200B;와(과) **[!UICONTROL 게시자]** 중에서 선택할 수 있습니다. 두 계정 역할 유형 간의 워크플로에서 유사점과 약간의 차이점을 확인하려면 [역할](/help/guide/overview/roles.md) 안내서를 참조하십시오.
* 계정의 **[!UICONTROL 업계]**&#x200B;를 선택하십시오. 일부 예로는 **[!UICONTROL 소매]**, **[!UICONTROL 통신]** 또는 **[!UICONTROL 금융 서비스]**&#x200B;가 있습니다.
...
>title="자사 디바이스 ID"
>abstract="ECID나 IP 주소와 같은 자사 디바이스 ID는 여러 개인이 공유할 수 있는 디바이스에 직접 연결됩니다."
...
* 소스에 **일반 텍스트 식별자**&#x200B;이(가) 포함된 경우 [데이터 연결 설정](./manage-data-connection.md#match-keys)에서 **[!UICONTROL 변환 적용]** 옵션을 사용하여 해시를 적용하세요. 이 옵션은 Experience Platform에서 대상을 소싱할 때만 사용할 수 있으며 클라우드 기반 소스에는 지원되지 않습니다.
...
**[!UICONTROL AdFixus ID]**&#x200B;을(를) 선택할 때 **[!UICONTROL 계정 자격 증명]** 섹션에서 외부 파트너에게 해당 ID를 제공해야 합니다. 이 옵션은 **[!UICONTROL AdFixus ID]**&#x200B;에서 전환하여 *after*&#x200B;에만 사용할 수 있습니다. **[!UICONTROL 계정 ID]** 필드에 AdFixus ID를 입력하여 값의 정확성을 다시 확인합니다.
...
>[!IMPORTANT]
>
>[!UICONTROL AdFixus ID]을(를) 변경해도 일치 키를 사용하여 기존 데이터 연결에 대해 [데이터 스케치](../glossary.md#sketches) 새로 고침이 트리거되지 않습니다. 데이터가 스케치되면 [데이터 연결 일정](./manage-data-connection.md#scheduling) 설정에 따라 다음 대상자가 새로 고쳐질 때까지 [!UICONTROL AdFixus ID]에 대한 변경 내용이 반영되지 않습니다. 다음 새로 고침 전에 변경해야 하는 경우 데이터 연결을 삭제하고 다시 만들 수 있습니다.
>
