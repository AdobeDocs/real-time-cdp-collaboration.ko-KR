...
조직을 추가하려면 왼쪽 레일에서 **[!UICONTROL 설정]**(으)로 이동하여 추가 아이콘(![추가 아이콘](/help/assets/icons/plus.png))을 선택하십시오. 오른쪽 상단에 있습니다. **[!UICONTROL 계정]**&#x200B;을(를) 선택합니다.
...
>title="사용자 ID"
>abstract="해시된 이메일 주소, 해시된 전화번호 또는 CRM ID와 같은 사용자 ID는 개별 프로필에 직접 연결됩니다."

>[!CONTEXTUALHELP]
>id="rtcdp_collaboration_organization_onboarding_deviceIDs"
>title="디바이스 ID"
>abstract="ECID나 IP 주소와 같은 디바이스 ID는 여러 개인이 공유할 수 있는 디바이스에 직접 연결됩니다."
...
다음 표에는 Collaboration에서 지원되는 일치 키가 나와 있습니다.

| 사용자 ID | 디바이스 ID | 파트너 ID |
| ------------- | ------------- | ------------- |
...
Collaboration은 사람 ID, 장치 ID 및 파트너 ID의 세 가지 일치 키 유형을 지원합니다. 모든 일치 키는 다음 요구 사항을 충족해야 합니다.
...
사용자 ID는 개별 프로필에 직접 연결됩니다. 현재 지원되는 ID:
...
장치 ID는 특정 장치에 연결된 식별자입니다. 현재 지원되는 ID:

* **[!UICONTROL 해시된 IPv4]**
* **[!UICONTROL IDFA]**: Apple iOS 장치에서 사용되는 IDFA(광고주용 식별자)입니다.
* **[!UICONTROL GAID]**: Android 장치에서 사용되는 Google 광고주 ID입니다.
* **[!UICONTROL ECID(Demdex ID)]**: 서드파티 쿠키가 활성화되면 ECID에 Adobe의 서드파티 쿠키 [!DNL Demdex ID]이(가) 포함됩니다. [!DNL Demdex ID]을(를) 사용하여 쿠키 기반의 인증되지 않은 방문자를 일치시킬 수 있습니다.
