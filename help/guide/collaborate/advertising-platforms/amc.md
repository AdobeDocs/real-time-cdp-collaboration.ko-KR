...
[!DNL Amazon Marketing Cloud]&#x200B;([!DNL AMC])과(와) 연결을 형성한 후 광고주는 [프로젝트를 만들고](../manage-projects.md#create-project)하여 [!DNL AMC]과(와) 공동 작업할 수 있습니다. [!DNL AMC] 프로젝트에서 **[!UICONTROL 검색]** 섹션을 사용하는 **대상 검색** 및 **[!UICONTROL 측정]** 탭을 사용하는 **측정** 사용 사례가 지원됩니다.
...
[!DNL AMC] 인스턴스에 캠페인 ID가 포함되어 있으면 **[!UICONTROL 측정값]** 탭을 사용할 수 있습니다. 프로젝트를 만들 때 Real-Time CDP Collaboration은 [!DNL AMC] 데이터에 대해 백그라운드 쿼리를 실행하여 [!UICONTROL Discover] 섹션과 측정 보고서를 구성하는 데 사용되는 캠페인 및 전환 이벤트 목록을 모두 채웁니다.

[!DNL AMC] 측정 보고서를 만들고 해석하는 방법에 대한 단계별 지침은 [AMC 측정 보고서 만들기](./amc-measure.md) 안내서를 참조하십시오.
