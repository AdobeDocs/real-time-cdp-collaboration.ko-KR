...
>[!IMPORTANT]
>
>겹치는 ID가 1000개 미만인 대상은 결과를 반환하지 않고 오류가 표시됩니다.
...
>[!NOTE]
>
>겹치기 비율 수치와 대상 색인 점수가 모든 대상에 대해 항상 사용 가능한 것은 아닐 수 있습니다. 겹침 비율과 대상 색인 점수의 가시성은 공동 작업자가 [메타데이터 가시성 섹션](/help/guide/setup/onboard-audiences.md#metadata-visibility)에서 대상에 대해 선택한 설정에 따라 다릅니다.

공동 작업자가 대상 인덱스 또는 중복 비율을 활성화하지 않은 경우 대상에는 사용 가능한 비교 데이터가 없습니다.
...
>[!NOTE]
>
>공동 작업자 대상의 가시성은 공동 작업자가 [연결 액세스 섹션](/help/guide/setup/onboard-audiences.md#connection-access) 및 [메타데이터 가시성 섹션](/help/guide/setup/onboard-audiences.md#metadata-visibility)의 대상에 대해 선택한 설정에 따라 다릅니다. 공동 작업자가 모든 대상을 비공개로 설정한 경우 이 섹션에는 대상이 표시되지 않습니다.
...
공동 작업자의 대상에 대해 대상 색인 점수가 활성화된 경우, 관련 대상은 대상 색인 점수를 기반으로 하며 대상 색인이 활성화되지 않은 대상은 포함되지 않습니다. 대상 색인 점수에 따른 관련 대상이 가장 높은 색인 점수가 먼저 표시되도록 정렬됩니다. 공동 작업자의 대상에 대해 대상 색인이 활성화되지 않은 경우 관련 대상은 겹침 백분율을 기반으로 합니다.
...
공동 작업자가 대상 인덱스 또는 중복 비율을 활성화하지 않은 경우 대상이 표시되지 않습니다.
...
![인덱스 점수를 계산하는 수식입니다.](/help/assets/collaborate/discover/index-score-formula.png)

자동차 제조업체가 대형 CTV 출판사와 함께 새로운 SUV 모델을 위한 광고 캠페인을 진행하려고 한다고 상상해 보십시오. 자동차 제조업체는 현재 유사한 모델을 소유하고 있는 사용자에 대한 데이터를 보유하고 있으며 이를 사용하여 고객에게 전환하기 위한 추가 잠재 고객을 찾고자 합니다. 자동차 제조업체는 CTV 게시자의 대상자를 살펴보고 현재 SUV 소유자와 밀접하게 일치하는 적절한 대상자를 찾는다.

![자동차 광고주와 CTV 게시자 대상 비교](/help/assets/collaborate/discover/audience-index-score-example.png)

색인 점수 계산은 이루어지며 캠페인의 성공 가능성을 판별하는 데 사용할 수 있습니다.
